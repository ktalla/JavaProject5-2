        ...
        cancelOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (spinner.getSelectedItem() != null) {
                    int index = Integer.parseInt(spinner.getSelectedItem().toString());
                    if (index < 0)
                        return;
                    StoreOrders storeOrders = singleton.getStoreOrders();
                    List<Order> orders = storeOrders.getOrders();
                    for (Order order : orders) {
                        Log.d("Order", "index: " + index + " " + order.getOrderNumber() + " " + order.toString());
                    ...
                    if (!singleton.getStoreOrders().getOrders().isEmpty()) {
